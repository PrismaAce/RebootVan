...
{
    public class ActiveRebootVanTile : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            RegisterItemDrop(ModContent.ItemType<RebootVanItem>());
            MinPick = 300;
        ...
        {
            // early return if no reboot card in inventory
            if (!Main.LocalPlayer.HasItem(ModContent.ItemType<RebootCardItem>()))
                return true;

            // early return if no reboot card held (this likely does nothing)
            if (Main.LocalPlayer.HeldItem.ModItem is not RebootCardItem)
                return true;

            
            if (Main.LocalPlayer.HeldItem.ModItem is RebootCardItem reboot)
            {
                // early return if unloaded reboot card
                if (!reboot.PlayerIsOnline) return true;

                for (int p = 0; p < Main.maxPlayers; p++)
                {
                    if (Main.player[p].active)
                    {
                        if (Main.player[p].name == reboot.PlayerName)
                        {

                            Vector2 telepoint = new(i * 16, j * 16);
                            Main.player[p].Teleport(telepoint, 11);
                            if (Main.netMode == NetmodeID.MultiplayerClient) SendKillPacket(Main.player[p].whoAmI, i * 16, j * 16);
                            else
                            {
                                if (Main.player[p].GetModPlayer<RebootPlayer>().GhostAtTheMoment)
                                {
                                    Main.player[p].GetModPlayer<RebootPlayer>().GhostAtTheMoment = false;
                                    Main.player[p].ghost = false;
                                    Main.player[p].GetModPlayer<RebootPlayer>().TBTHardcore = true;
                                    PrismaUtils.DisplayText(reboot.PlayerName + " was rebooted");

                                }
                                Main.player[p].respawnTimer = 1;
                                Main.player[p].Teleport(telepoint, 11);
                                NetMessage.SendData(MessageID.TeleportEntity, -1, -1, null, 0, Main.player[p].whoAmI, telepoint.X, telepoint.Y, 1);
                            }
            ...
            for (int k = 0; k < 8; k++)
            {
                for (int l = 0; l < 6; l++)
                {
                    Main.tile[topX + k, topY + l].TileType = (ushort)ModContent.TileType<RebootVanTile>();
                }
            }
            ...
            //Main.LocalPlayer.ConsumeItem(ModContent.ItemType<RebootCardItem>(), true);
            Main.LocalPlayer.HeldItem.TurnToAir();
            SoundStyle ChugJug = new SoundStyle($"{nameof(RebootVan)}/Sounds/ReviveSFX")
            {
                Volume = 0.8f,
                Pitch = 0f
            };
        ...
        {
            Main.LocalPlayer.cursorItemIconID = ModContent.ItemType<RebootCardItem>();
        ...
        private void SendKillPacket(int who, int tpx, int tpy)
        {
